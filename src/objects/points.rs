...
impl Point<Offset> {
    ...
    pub fn new(x: i32, y: i32) -> Point<Offset> {
        (x, y).into()
    }
    pub fn offset(x: i32, y: i32) -> Point<Offset> {
        (x, y).into()
    }
    pub fn axial(x: i32, y: i32) -> Point<Axial> {
        (x, y).into()
    }
    pub fn cubic(x: i32, y: i32, z: i32) -> Point<Cubic> {
        (x, y, z).into()
    }
    pub fn index(i: Index) -> Point<Offset> {
        Point::from_index(i)
    }

    pub fn subtype(&self) -> SystemType {
        SystemType::Offset
    }
    ...
    pub fn to_axial(self) -> Point<Axial> {
        self.into()
    }
    pub fn to_cubic(self) -> Point<Cubic> {
        self.into()
    }

    pub fn as_axial(&self) -> Point<Axial> {
        self.into()
    }
    pub fn as_cubic(&self) -> Point<Cubic> {
        self.into()
    }
    ...
    pub fn bounds(&self) -> (Point<Offset>, Point<Offset>) {
        let (w, h) = Context::size();
        (Point::new(-w / 2, -h / 2), Point::new(w / 2, h / 2))
    }

    pub fn in_bounds(&self) -> bool {
        let (s, e) = self.bounds();
        let (x, y) = self.integers();
        s.x <= x && s.y <= y && e.x > x && e.y > y
    }
}
...
impl Point<Axial> {
    ...
    pub fn to_index(self) -> Index {
        self.to_offset().as_index()
    }
    pub fn to_offset(self) -> Point<Offset> {
        self.into()
    }
    pub fn to_cubic(self) -> Point<Cubic> {
        self.into()
    ...
    pub fn as_index(&self) -> Index {
        self.as_offset().as_index()
    }
    pub fn as_offset(&self) -> Point<Offset> {
        self.into()
    }
    pub fn as_cubic(&self) -> Point<Cubic> {
        self.into()
...
impl Point<Cubic> {
    ...
    pub fn integers(&self) -> (i32, i32, i32) {
        self.into()
    }
...
mod tests {
    ...
    macro_rules! axial {
        ( $x:expr, $y:expr ) => {
            Point::offset($x, $y).to_axial()
        };
    }

    macro_rules! cubic {
        ( $x:expr, $y:expr ) => {
            Point::offset($x, $y).to_cubic()
        };
    }
    ...
    fn point_neighbors_30x30_corners() {
        ...
        // bottom-right corner
        assert_eq!(point!(14, -15).neighbors(), points![(14, -14), (13, -15)],);

        // top-left corner
        assert_eq!(point!(-15, 14).neighbors(), points![(-15, 13), (-14, 14)],);
