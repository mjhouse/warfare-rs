...
static MALE: Lazy<Vec<String>> = Lazy::new(|| {
    let mut result: Vec<String> = BufReader::new(
        File::open(MALE_NAMES)
            .expect("Cannot find male names"))
        .lines()
        .into_iter()
        .filter(Result::is_ok)
        .map(Result::unwrap)
...
static FEMALE: Lazy<Vec<String>> = Lazy::new(|| {
    let mut result: Vec<String> = BufReader::new(
        File::open(FEMALE_NAMES)
            .expect("Cannot find female names"))
        .lines()
        .into_iter()
        .filter(Result::is_ok)
        .map(Result::unwrap)
...
static LAST: Lazy<Vec<String>> = Lazy::new(|| {
    let mut result: Vec<String> = BufReader::new(
        File::open(LAST_NAMES)
            .expect("Cannot find last names"))
        .lines()
        .into_iter()
        .filter(Result::is_ok)
        .map(Result::unwrap)
